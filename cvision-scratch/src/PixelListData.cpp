#include "PixelListData.h"
        ...
        PixelList<BGRPixel> PixelListData<BGRPixel>::convert_to_pixel_list(cv::Mat image){
            
            PixelList<BGRPixel> temp_pixel_list; 
            for (int i = 0; i < image.rows; ++i) {
                for (int j = 0; j < image.cols; ++j) {
                    cv::Vec3b temp = image.at<cv::Vec3b>(i,j);
                    temp_pixel_list.push_back(std::make_pair(cv::Point(j,i), temp)); 
                }
            }
            return temp_pixel_list;
        }
        ...
        bool PixelListData<BGRPixel>::display(std::string const &win_name){
        ...
        bool PixelListData<BGRPixel>::save_image(const char *name){
            cv::Mat temp = get_image();
            if(!temp.data){
                return false;
            }
