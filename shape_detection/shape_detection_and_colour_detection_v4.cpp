...
int imageProcessing(int shape_type, Mat imgInput, int minimum_area, int *detected_center_x, int *detected_center_y, double *contour_area, double* detected_radius);
...
int main( int argc, char** argv ){
	...
	while (true){
		...
		// buoy_number = imageProcessing(12, imageRead, MAX_CONTOUR_SIZE, &center_buoy_x, &center_buoy_y, &buoy_area, &radius_buoy);
...
int imageProcessing(int shape_type, Mat imgInput, int minimum_area, int *detected_center_x, int *detected_center_y, double *contour_area, double* detected_radius){
	...
	for (int i = 0; i < contours.size(); i++){
		...
		else if (shape_type == 12 && approx.size() == 12 && std::fabs(cv::contourArea(contours[i])) > minimum_area)
		{
			double a = contourArea( contours[i],false);  //  Find the area of contour
			if(a > largest_shape_area && a > minimum_area){
				largest_shape_area = a;
